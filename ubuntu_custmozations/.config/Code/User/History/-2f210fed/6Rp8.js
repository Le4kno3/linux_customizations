...
task("get-balance", "Reads the number")
  ...
  .setAction(async (taskArgs) => {
    ...
    const myHalbornToken = await new ethers.Contract(contractAddr, MyHalbornToken.interface, signer)
...
// We require a provider to query the network
    ...
    let steve = new ethers.Wallet(privateKey1, provider);

    let privateKey2 = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d"
    let takshil = new ethers.Wallet(privateKey2, provider);

    let privateKey3 = "0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a"
    let bob = new ethers.Wallet(privateKey3, provider);
    
    let steve_balance = BigInt(await myHalbornToken.balanceOf(steve.address)).toString()
    let takshil_balance = BigInt(await myHalbornToken.balanceOf(takshil.address)).toString()
    let bob_balance = BigInt(await myHalbornToken.balanceOf(bob.address)).toString()
    ...
    // await myHalbornToken.transfer(takshil.address, someamount);
