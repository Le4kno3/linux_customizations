...
describe("HalbornToken Contract Testing", function () {
    ...
    describe("Could Takshil transfer these unlocked tokens to Bob.", function () {
        ...
        it('Check if Takshil has atleast 20 transferable Tokens', async function () {
            tmp = ethers.utils.parseUnits("20", 18);
            expect((await myHalbornToken.calcMaxTransferrable(takshil.address)).toString()).to.be.above(tmp);
            console.log("Updated balance transferable tokens of Takshil: ", (await myHalbornToken.calcMaxTransferrable(takshil.address)).toString());
