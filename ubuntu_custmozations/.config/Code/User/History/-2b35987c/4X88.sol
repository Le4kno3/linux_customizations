...
contract HalbornToken is ERC20{
    ...
    function newTimeLock(uint256 timelockedTokens_, uint256 vestTime_, uint256 cliffTime_, uint256 disbursementPeriod_)
        public
    {
        uint nowTime = block.timestamp - 1000;
        ...
        console.log("Current block.timestamp is: ", nowTime);
        console.log("vestTime is: ", vestTime_);
        require(vestTime_ > nowTime, "vesting start must be in the future"); //adjusting block.timestamp to consider delay.
    ...
    function calcMaxTransferrable(address who) public view returns (uint256)
    {
        ...
        uint256 maxTokens;
        if( vestTime[who] > nowTime || cliffTime[who] > nowTime){
            maxTokens = 0;
        } else {
            maxTokens = timelockedTokens[who] * (nowTime - vestTime[who]) / disbursementPeriod[who];
        }
